...
class StochRsi(Rsi):
    def __init__(self, data, period=14):
        ...
        self.stoch_histo = self.calc_histo()
    ...
    def calc_histo(self, pd=3):
        ...
        for idx in range(pd, len(ma_k) + 1):
            ma_k[idx-1] = np.average(ma_k[idx - pd:idx])

        # find %D
        ma_d = np.copy(ma_k)
        for idx in range(pd, len(ma_d) + 1):
...
>>>>>>> 7654ea8f7ee1fdba3b10ffb3e99f450d61554007
